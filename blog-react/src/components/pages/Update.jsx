import React, { useState, useEffect, Children } from 'react'
import { useForm } from '../../hooks/useForm'
import { Request } from '../../helpers/Request';
import { Url } from '../../helpers/Url';
import { useParams } from 'react-router-dom';

export const Update = () => {

  const { object, change, submit } = useForm({});
  const [updated, setupdated] = useState("no_enviado");
  const [article, setArticle] = useState({});
  const params = useParams();

  useEffect(() => {
    getArticle();
  }, [])

  const getArticle = async () =>{

    const {data} = await Request(Url.urlGetById + params.id, "GET");
    if (data.status === "Success") {
      setArticle(data.article);
    }
  }

  const updateArticle = async e => {
    e.preventDefault();

    let articleAux = object;

    if (!object.title) {
      articleAux = {
        title: article.title,
        content: object.content
      }
    }

    if (!object.content) {
      articleAux = {
        ...articleAux,
        content: article.content
      }
    }

    

    console.log(articleAux)

    const { data } = await Request(Url.urlUpdateArticle + params.id, "PUT", articleAux);
    if (data.status === "Success") {

      let fileInput = document.querySelector("#file");
      if (fileInput.files[0]) {
        let formData = new FormData();
        formData.append("file", fileInput.files[0]);
        let upload = await Request(Url.urlUploadImage+data.articleUpdated._id, "POST", formData, true);
      }
      setupdated(data.status);
    }else{

      setupdated(data.status);
    }
  }

  return (
    <div className='jumbo'>

      <h1>Editar Articulo</h1>
      <strong>{updated === "Success" ? "Actualizado correctamente !" : ""}</strong>
      <strong>{updated === "Error" ? "Error en los datos enviados !" : ""}</strong>

      <form className='form' onSubmit={updateArticle}>

        <div className='form-group'>
          <label htmlFor='title'>Titulo</label>
          <input type='text' name='title' onChange={change} defaultValue={article.title} />
        </div>

        <div className='form-group'>
          <label htmlFor='content'>Contenido</label>
          <textarea name='content' onChange={change} defaultValue={article.content} />
        </div>

        <div className='form-group'>
        
        </div>

        <div className='form-group'>
          <div className='mask'>
            {article.image != "default.png" && <img src={Url.urlGetImage + article.image} />}
            {article.image == "default.png" && <img src='data:image/png;base64,' />}
          </div>
          <label htmlFor='file'>Imagen</label>
          <input type='file' name='file' id='file'/>
        </div>

        <div className='form-group'>
          <input type='submit' value="Actualizar"/>
        </div>

      </form>

    </div>
  )
}
