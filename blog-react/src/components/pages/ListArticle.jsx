import React from 'react'
import { Url } from '../../helpers/Url'
import { Request } from '../../helpers/Request'
import { Link } from 'react-router-dom'

export const ListArticle = ({articles, setArticles}) => {

    const deleteArticle = async (id) => {
        let {data} = await Request(Url.urlDeleteArticle + id, "DELETE");
        if (data.status === "Success") {
            let articlesAux = articles.filter(article => article._id !== id);
            setArticles(articlesAux);
        }
    }

    return (
        articles.map(article => {
            return (
                <article className='article-item' key={article._id}>
                    <div className='mask'>
                        {article.image != "default.png" && <img src={Url.urlGetImage + article.image} />}
                        {article.image == "default.png" && <img src='data:image/png;base64,' />}
                    </div>
                    <div className='data'>
                        <h3><Link to={"/article/" + article._id}>{article.title}</Link></h3>
                        <p>{article.content}</p>
                        <button className='edit'><Link to={"/update/" + article._id}>Editar</Link></button>
                        <button className='delete' onClick={() => deleteArticle(article._id)}>Borrar</button>
                    </div>
                </article>
            )
        })
    )
}
